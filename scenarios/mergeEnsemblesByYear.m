function mergeEnsemblesByYear(dataPath, hour, position, month,years, boundaries, outputFolder, palette, saving_quality, cont, Heights)
...
colorbar;
hold on
firstFile = strcat(num2str(years(1)),'-',num2str(month,'%02.f'),'-',num2str(1,'%02.f'),'_h_',num2str(hour,'%02.f'),'_P',num2str(position),'_',num2str(cont),'.mat');
...
for year= 1:length(years)
    for iter = 2:29
        FileName = strcat(num2str(years(year)),'-',num2str(month,'%02.f'),'-',num2str(iter,'%02.f'),'_h_',num2str(hour,'%02.f'),'_P',num2str(position),'_',num2str(iter),'.mat');
